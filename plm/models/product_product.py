...
# _name = "product.product"  # removed for v19 compatibility
    ...
    def check_product_change_impact(self):
        ...
        return self.env.ref("plm.action_report_product_change_impact").report_action(
            docids=[self.id], data=report_data
        )
    ...
    def _father_part_compute(self, name="", arg={}):
        ...
        for prod_obj in prod_objs:
            ...
            for bom_line_obj in bom_line_objs:
                for objPrd in self.search(
                    [("product_tmpl_id", "=", bom_line_obj.bom_id.product_tmpl_id.id)]
                ):
    ...
    def _computeStd(self):
        for product_product_id in self:
            ...
            if product_product_id.std_description:
                if (
                    product_product_id.std_description.umc1
                    or product_product_id.std_description.fmt1
                ):
                    product_product_id.show_std_field1 = True
                if (
                    product_product_id.std_description.umc2
                    or product_product_id.std_description.fmt2
                ):
                    product_product_id.show_std_field2 = True
                if (
                    product_product_id.std_description.umc3
                    or product_product_id.std_description.fmt3
                ):
    ...
    @api.onchange(
        "std_value1", "std_value2", "std_value3", "std_umc1", "std_umc2", "std_umc3"
    )
    def on_change_stdvalue(self):
        if self.std_description:
            self.name = self.computeDescription(
                self.std_description,
                self.std_umc1,
                self.std_umc2,
                self.std_umc3,
                self.std_value1,
    ...
    def getParentBomStructure(self, filterBomType=""):
        ...
        for mrpBomLine in mrpBomLines:
            out.append(
                (
                    self.env["mrp.bom"].where_used_header(mrpBomLine),
    ...
    def getLatestReleasedRevision(self):
        for product_id in self.search(
            [("engineering_code", "=", self.engineering_code)],
            order="engineering_code desc",
        ):
    ...
    def getParentBom(self, filterBomType="normal"):
        for product_product_id in self:
            ...
            mrp_bom_line_ids = self.env["mrp.bom.line"].search(bom_line_filter, limit=1)
            for mrp_bom_line_id in mrp_bom_line_ids:
                if (
                    product_product_id.id
                    != mrp_bom_line_id.bom_id.product_tmpl_id.product_variant_id.id
                ):
    ...
    def computeDescription(
        ...
        if thisObject.fmtend:
            ...
            description = (
                description
                + " "
                + self._packfinalvalues(
                    thisObject.fmtend, description1, description2, description3
                )
            )
        else:
            if std_umc1 and std_value1:
                description = (
                    description
                    + " "
                    + self._packvalues(thisObject.fmt1, std_umc1, std_value1)
                )
            if std_umc2 and std_value2:
                description = (
                    description
                    + " "
                    + self._packvalues(thisObject.fmt2, std_umc2, std_value2)
                )
            if std_umc3 and std_value3:
                description = (
                    description
                    + " "
                    + self._packvalues(thisObject.fmt3, std_umc3, std_value3)
                )
    ...
    def product_template_open(self):
        ...
        search_res = mod_obj.get_object_reference(
            "plm", "product_template_form_view_plm_custom"
        )
    ...
    def _getChildrenBom(self, product_product_id, level=0, currlevel=0, bom_type=False):
        ...
        #
        def get_flat_children(product_product_id, level=0, currlevel=0, bom_type=False):
            ...
            for bomid in product_product_id.product_tmpl_id.bom_ids:
                ...
                for bomline in bomid.bom_line_ids:
                    children_product_product_ids = get_flat_children(
                        product_product_id=bomline.product_id,
                        level=level,
        ...
        #
        result = get_flat_children(product_product_id, level, currlevel, bom_type)
    ...
    def getLeafBom(self, bom_type="normal"):
        ...
        def _getLeafBom(parent_product_ids, bom_type="normal"):
            for product_product_id in parent_product_ids:
                has_bom = False
                for mrp_bom_id in product_product_id.product_tmpl_id.bom_ids.filtered(
                    lambda x: x.type == bom_type
                ):
                    has_bom = True
            ...
            for bom_id in product_product_id.product_tmpl_id.bom_ids:
                if bom_type:
                    if bom_id.type != bom_type:
                        continue
                out[bom_id] = bom_id.summarize_level(
                    recursion, flat, level, summarize, parentQty, bom_type
                )
        return out
    ...
    def GetLatestIds(self, vals, forceCADProperties=False):
        ...
        def getCompIds(partName, partRev):
            if docRev is None or docRev is False:
                partIds = self.search(
                    [("engineering_code", "=", partName)], order="engineering_revision"
                ).ids
                if len(partIds) > 0:
    ...
    def SaveOrUpdate(self, vals):
        ...
        for partVals in vals:
            ...
            if "engineering_code" not in partVals:
                ...
                continue
            existingCompBrwsList = self.search(
                [
                    ("engineering_code", "=", partVals["engineering_code"]),
                    (
                        "engineering_revision",
                        "=",
                        partVals.get("engineering_revision", 0),
                    ),
                ],
                order="engineering_revision ASC",
            )
                    ...
                    if self._iswritable(existingCompBrws):
                        ...
                        if weight:
                            if not self.write({"weight": weight}):
                                raise UserError(
                                    _(
                                        "Part %r cannot be updated"
                                        % (partVals["engineering_code"])
                                    )
                                )
                        else:
                            logging.warning(
                                "No Weight property set unable to update !!"
                            )
    ...
    def QueryLast(self, request=([], []), default=None):
        ...
        allIDs = self.search(queryFilter, order="engineering_revision").ids
        if len(allIDs) > 0:
            allIDs.sort()
    ...
    def _get_recursive_parts(self, include_statuses):
        ...
        for prodBrws in self:
            for childProdBrws in self.browse(
                self._getChildrenBom(prodBrws, 1)
            ).filtered(
                lambda x: x.engineering_code not in [False, ""]
    ...
    def _jump_document_wf(self, documentBrws, check_state):
        """
    ...
    def checkWorkflow(
        self, docInError, linkeddocuments, check_state, check_in_check=True
    ):
        docIDs = []
        ...
        for documentBrws in linkeddocuments:
            if documentBrws.engineering_state in check_state:
                ...
                if documentBrws.is3D():
                    ...
                    raw_doc_ids = documentBrws.getRelatedRfTree(
                        documentBrws.id, recursion=True
                    )
                    docIDs.extend(
                        self.checkWorkflow(
                            docInError,
    ...
    def customMoveDocumentWorkflow(self, ir_attachment_ids, status):
        """
    ...
    def _iswritable(self, oid):
        ...
        if not oid.engineering_writable:
            logging.warning(
                "_iswritable : Part (%r - %d) is not writable."
                % (oid.engineering_code, oid.engineering_revision)
            )
            return False
        if oid.engineering_state not in checkState:
            logging.warning(
                "_iswritable : Part (%r - %d) is in status %r."
                % (
                    oid.engineering_code,
                    oid.engineering_revision,
        ...
        if not oid.engineering_code:
            logging.warning(
                ...
                % (oid.name, oid.engineering_revision)
            )
            return False
        return True
        ...
        for checkObj in self:
            ...
            for oldObject in prod_ids:
                ...
                if not oldObject.with_context(ctx).write(values):
                    msg = (
                        "Unlink : Unable to update state in product Engineering Code = %r   Engineering Revision = %r   Document Id = %r"
                        % (
                            oldObject.engineering_code,
                            oldObject.engineering_revision,
                            oldObject.id,
                        )
                    )
                    logging.warning(msg)
                    raise UserError(
                        _(
                            "Cannot restore previous product Engineering Code = %r   Engineering Revision = %r   Document Id = %r"
                            % (
                                oldObject.engineering_code,
                                oldObject.engineering_revision,
                                oldObject.id,
    ...
    def _getbyrevision(self, name, revision):
        return self.search(
            [("engineering_code", "=", name), ("engineering_revision", "=", revision)]
        )
    ...
    def action_confirm(self):
        ...
        """
        for product_product_id in self:
            product_product_id.commonWFAction(
                CONFIRMED_STATUS, [START_STATUS], recursive=True
            )
        return True
    ...
    def action_release(self):
        for product_product_id in self:
            product_product_id.commonWFAction(
                RELEASED_STATUS, [CONFIRMED_STATUS], recursive=True
            )
        return True
    ...
    def action_obsolete(self):
        ...
        for product_product_id in self:
            ...
            product_product_id.commonWFAction(status, include_statuses, recursive=False)
        return True
    ...
    def commonWFAction(
        self, status, include_statuses=[], recursive=False, check_in_check=True
    ):
        performed_ids = []

        def _commonWFAction(
            obj, status, include_statuses=[], recursive=False, check_in_check=True
        ):
            for product_product_id in obj:
                ...
                #
                if recursive:
                    errors, product_ids = product_product_id._get_recursive_parts(
                        include_statuses
                    )
                    if len(errors) > 0:
        ...
        return _commonWFAction(
            self, status, include_statuses, recursive, check_in_check=check_in_check
        )
    ...
    def checkVariantLinkeddocs(self, doc_ids):
        for doc in self.env["ir.attachment"].browse(doc_ids):
            if not doc.ischecked_in():
                raise UserError(
                    _(
                        "Document %r with revision %r is in check-out, cannot create variant."
                        % (doc.name, doc.engineering_revision)
                    )
                )
    ...
    def create(self, vals):
        ...
        for vals_dict in vals:
            ...
            if eng_code:
                ...
                if prodBrwsList:
                    raise UserError(
                        "Component %r already exists" % (vals_dict["engineering_code"])
                    )
        ...
        try:
            ...
            if isinstance(ex, psycopg2.IntegrityError):
                ...
                raise Exception(msg)
            logging.error(
                "(%s). It has tried to create with values : (%s)."
                % (str(ex), str(vals))
            )
            raise Exception(
                _(" (%r). It has tried to create with values : (%r).") % (ex, vals)
            )

    def write(self, vals):
        for product in self:
            if "is_engcode_editable" not in vals and product.engineering_code not in [
                "-",
                False,
            ]:
            ...
            if not product.description or (
                "description" in vals and not vals["description"]
            ):
                vals["description"] = "."
    ...
    def read(self, fields=[], load="_classic_read"):
        try:
            if self.env.context.get("odooPLM"):
                ...
                customFields = [
                    field.replace("plm_m2o_", "")
                    for field in cleaned_up_fields
                    if field.startswith("plm_m2o_")
                ]
...
%r\n
Please try to contact OmniaSolutions to solve this error, or install Plm Sale Fix module to solve the problem.""" % (
                    ex
                )
            raise ex
                    ...
                    if field_def.get("type", "") == "many2one":
                        ...
                        if relation:
                            if isinstance(fieldVal, int):
                                tmpVals[customField] = (
                                    self.env[relation].browse(fieldVal).name
                                )
    ...
    def checkMany2oneClient(self, vals, force_create=False):
        return self._checkMany2oneClient(
            self.env["product.product"], vals, force_create
        )
    ...
    def _checkMany2oneClient(self, obj, vals, force_create=False):
        ...
        if customFields:
            ...
            for fieldName, fieldDefinition in fieldsGet.items():
                ...
                if not field_id:
                    refId = self.customFieldConvert(
                        fieldDefinition, vals, fieldName, force_create=force_create
                    )
                    out[fieldName] = False
                    if refId:
    ...
    def translateForClient(self, values=[], forcedLang=""):
        ...
        if language and values:
            toRead = [
                x for x in list(values.values()) if type(x) in [str] and x
            ]  # Where computed only string and not null string values (for performance improvement)
            ...
            for fieldName, valueToTranslate in list(values.items()):
                ...
                translationBrwsList = translationObj.search(
                    [("lang", "=", language), ("src", "=", valueToTranslate)]
                )
    ...
    def action_rev_docs(self):
        ...
        for compBrws in self:
            ...
            if not engineering_code:
                logging.warning(
                    "Part %s doesn't have and engineering code!" % (compBrws.name)
                )
                docIds.extend(compBrws.linkeddocuments.ids)
            else:
                compBrwsList = self.search(
                    [("engineering_code", "=", engineering_code)]
                )
    ...
    def action_view_mos(self):
        ...
        logging.warning(
            "[action_view_mos] product with id %s does not have a related template"
            % (self.id)
        )
    ...
    def NewRevision(self):
        ...
        for tmpObject in self:
            ...
            for product_product_id in self.browse(latestIDs[-1]):
                ...
                available_status = self._fields.get(
                    "engineering_state"
                )._description_selection(self.env)
                dict_status = dict(available_status)
                status_lable = dict_status.get(product_product_id.engineering_state, "")
                product_product_id.message_post(
                    body=_(
                        "Status moved to: %s by %s."
                        % (status_lable, self.env.user.name)
                    )
                )
                # store updated infos in "revision" object
                defaults = {}
                defaults["name"] = (
                    product_product_id.name
                )  # copy function needs an explicit name value
                ...
                new_tmpl_id = product_product_id.product_tmpl_id.with_context(ctx).copy(
                    defaults
                )
    ...
    def getNormalBomStd(self):
        ...
        bomType = "normal"
        logging.info(
            "getNormalBom for compoent: %s, componentRev: %s"
            % (componentName, componentRev)
        )
        out = []
        searchFilter = [
            ("engineering_code", "=", componentName),
    ...
    def _getChildrenBomWithDocuments(self, component, level=0, currlevel=0):
        ...
        for bomid in component.product_tmpl_id.bom_ids:  # TODO: Bom type??
            for bomline in bomid.bom_line_ids:
                ...
                levelDict["bom"] = self._getChildrenBom(
                    bomline.product_id, level, currlevel + 1
                )
    ...
    def computeLikedDocuments(self, prodBrws):
        ...
        for docBrws in prodBrws.linkeddocuments:
            docList.append(
                {"component": compInfos, "document": docBrws.getDocumentInfos()}
            )
        return docList

    def getComponentInfos(self):
        return {
            "engineering_code": self.engineering_code or "",
    ...
    def getCloneRevisionStructure(self, values=[]):
        ...
        computeType = values[
            1
        ]  # Possible values = 'no-bom' / 'bom-all-levels' / 'bom-one-level'
        eng_code = componentDict.get("engineering_code", "")
        eng_rev = componentDict.get("engineering_revision", None)
        if not eng_code:
            logging.warning(
                "No engineering code passed by the client %r" % (componentDict)
            )
            return {}
        if eng_rev is None:
            logging.warning(
                "No engineering revision passed by the client %r" % (componentDict)
            )
            return {}
        componentId = componentDict.get("_id", None)
        if not componentId:
            prodBrws = self.search(
                [
                    ("engineering_code", "=", eng_code),
                    ("engineering_revision", "=", eng_rev),
                ]
        ...
        if computeType == "no-bom":
            ...
            rootProps["can_revise"] = (
                True  # Because cames from previous new revision. So it can be revised
            )
            return {
    ...
    def reviseCompAndDoc(self, elementsToClone):
        ...
        def restorePreviousDocumentRevision(node):
            node["DOCUMENT_ATTRIBUTES"]["engineering_revision"] = (
                node["DOCUMENT_ATTRIBUTES"]["engineering_revision"] - 1
            )
            return docEnv.getDocumentBrws(
                node["DOCUMENT_ATTRIBUTES"]
            )  # Try to restore previous revision (case of only document revision)

        def restorePreviousComponentRevision(node):
            node["PRODUCT_ATTRIBUTES"]["engineering_revision"] = (
                node["PRODUCT_ATTRIBUTES"]["engineering_revision"] - 1
            )
            return self.getCompBrws(
        ...
        def updateRootDocument(docBrws, node):
            if docBrws:
                ...
                newDocBrws.desc_modify = node["DOCUMENT_ATTRIBUTES"].get(
                    "desc_modify", ""
                )
                return newDocBrws
            return False
        ...
        def reviseComp(
            node, isRoot, compBrws, docBrws, reviseDocument, reviseComponent
        ):
            newCompBrwse = False
            newDocBrws = False
            deleted = False
        ...
        def setupRelations(newDocBrws, newCompBrwse):
            if newDocBrws:
                newDocBrws.write(
                    {"linkedcomponents": [(5, 0, 0)]}
                )  # Clean copied links
                if newCompBrwse:
                    newCompBrwse.write(
                        {"linkeddocuments": [(4, newDocBrws.id, False)]}
        ...
        def nodeResursionUpdate(node, isRoot=False):
            ...
            if engCode:
                newCompBrwse, newDocBrws = reviseComp(
                    node, isRoot, compBrws, docBrws, reviseDocument, reviseComponent
                )
            elif docName:
            ...
            for childNode in node.get("RELATIONS", []):
                if (
                    childNode.get("DOCUMENT_ATTRIBUTES", {}).get("DOC_TYPE").upper()
                    == "2D"
                ):
                    childNode["PRODUCT_ATTRIBUTES"] = {
                        "engineering_code": ""
                    }  # Setup the correct parent component
    ...
    def cloneCompAndDoc(self, elementsToClone):
        ...
        def createClonedRawComponent(engCode, compBrws, node):
            ...
            default = {"name": newCode, "engineering_code": newCode}
        ...
        def cloneDocumentObj(oldDocBrws, engCode, node):
            ...
            if oldDocBrws.id:
                ...
                node["DOCUMENT_ATTRIBUTES"][
                    "CHECK_OUT_BY_ME"
                ] = oldDocBrws._is_checkedout_for_me()
            return newDocBrws
        ...
        def cloneWithComp(
            ...
            if not cloneComponent:
                ...
                if not isRoot:  # Part-Part case because BOM is not managed
                    rootEngCode, newRawComponent = createClonedRawComponent(
                        rootEngCode, compBrws, node
                    )
            if cloneDocument:
        ...
        def nodeResursionUpdate(node, isRoot=False):
            ...
            if compProps.get(
                "engineering_code", ""
            ):  # Node is a component-document node
                newDocBrws, newRawComponent = cloneWithComp(
                    node.get("COMPONENT_CHECKED", False),
                    cloneDocument,
                    rootEngCode,
                    oldDocBrws,
                    compBrws,
                ...
                newDocBrws = cloneWithDoc(
                    cloneDocument, oldDocBrws, node, isRoot, baseName
                )
    ...
    def getComponentBrws(self, componentVals):
        ...
        if not engCode or engRev is None:
            return self.browse()
        return self.search(
            [("engineering_code", "=", engCode), ("engineering_revision", "=", engRev)]
        )

    def open_related_revisions(self):
        product_product_ids = self.search(
            [("engineering_code", "=", self.engineering_code)]
        )
        return {
            "name": _("Products"),
    ...
    def createFromProps(self, productAttribute):
        ...
        for product_produc_id in self.search(
            [
                ("engineering_code", "=", engineering_name),
                (
                    "engineering_revision",
                    "=",
            ...
            break
        if found:  # Write
            if product_produc_id.engineering_state not in [
                RELEASED_STATUS,
                OBSOLATED_STATUS,
            ]:
    ...
    def _name_search(self, name, domain=None, operator="ilike", limit=None, order=None):
        ...
        product_ids = list(
            super(ProductProduct, self)._name_search(
                name, domain, operator, limit, order=None
            )
        )
        ...
        def computeChildLevel(product_tmpl_id):
            ...
            if product_tmpl_id.id not in product_computed:
                ...
                for mrp_bom_line_id in mrp_bom_id.bom_line_ids:
                    children.append(
                        (
                            getDictData(mrp_bom_line_id),
                            computeChildLevel(
                                mrp_bom_line_id.product_id.product_tmpl_id
                            ),
                        )
                    )
            return children
    ...
    def get_product_bom_flat_ids(self, bom_type="normal"):
        ...
        def populate(product_id):
            ...
            for bom_id in product_id.product_tmpl_id.bom_ids.filtered(
                lambda x: x.type == bom_type
            ):
    ...
    def get_all_document_source_path(self, attributes, *k, **kw):
        ...
        def fillUpSrvPath(p_p_id):
            for ir_attachment_id in p_p_id.linkeddocuments.filtered(
                lambda x: x.document_type == "3d"
            ):
                is_brake = False
                for sub_ir_attachment_id in ir_attachment_id.GetRelatedDocs(
                    getBrowse=True
                ):
                ...
                if is_brake:
                    continue
                out_src.append(
                    (ir_attachment_id.first_source_path, ir_attachment_id.id)
                )
